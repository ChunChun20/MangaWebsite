...
const MangaDetailsPage = () => {
    ...
    const fetchMangaDetails = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://127.0.0.1:5000/mangas/${mangaId}`,{
                headers: {
            ...
            if (response.data.manga.isFavourite != null) {
                setIsFavourite(response.data.manga.isFavourite)
            }
    ...
    const handleRatingChange = async (newRating) => {
        if(!user){
          ...
          try{
             await axios.post(`http://127.0.0.1:5000/mangas/${mangaId}/rate`,{
                 rating: newRating
             },
              {
                headers: {
       ...
       const handleAddToFavourites = async () => {
        if(!user){
            ...
            try{
            await axios.post(`http://127.0.0.1:5000/mangas/${mangaId}/favourite`, {}, {
                   headers: {
                       'Authorization': `Bearer ${token}`
                   }
               });
